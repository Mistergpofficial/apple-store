...
<head>
    ...
    <title><?php echo config('siteconfig.topgrosapps_title') ?> - <?php echo config('siteconfig.site_title') ?></title>
    <meta name="description" content="<?php echo config('siteconfig.topgrosapps_title') ?> - <?php echo config('siteconfig.site_title'); ?>" />
    ...
    <meta property="og:title" content="<?php echo config('siteconfig.topgrosapps_title') ?> - <?php echo config('siteconfig.site_title'); ?>"/>
    <meta property="og:description" content="<?php echo config('siteconfig.topgrosapps_title') ?> - <?php echo config('siteconfig.site_title'); ?>"/>
    <!-- CSS and Scripts -->
    ...
    <div class="container">
        ...
        <div class="pageresults">
            ...
            <ul class="page-itemlist">
                @if($topappgross->count() > 0)
                    @foreach($topappgross as $topappgro)
                        <li class="page-item">
                                <div class="pagethumb" data-toggle="tooltip" data-placement="top" title="{{ $topappgro->imname1 }}">
                                    <a href="{{ url('/app/' . $topappgro->musicid1[0] . '/' . cano($topappgro->imname1)) }}">
                                        <img data-src="{!! asset($topappgro->bigimage1) !!}" src="{!! asset('public/images/loading.svg') !!}" >
                                    </a>
                                </div>
                                <div class="info">
                                    <h3>
                                        <a href="{{ url('/app/' . $topappgro->musicid1[0] . '/' . cano($topappgro->imname1)) }}">{{ $topappgro->imname1 }}</a>
                                    </h3>
                                    <h4>{{ $topappgro->imartist1 }}</h4>
                                    <a class="genre" href="{{ url('/category/' . $topappgro->catid1[0] . '/' . cano($topappgro->label)) }}">{{ $topappgro->label }}</a>
                                    <span class="summary">{{ $topappgro->summary }}</span>
                                </div>
                            </li>
                    @endforeach
                        </ul>
                ...
                @else
                    @php
                        ...
                        $string1 = file_get_contents('https://itunes.apple.com/'.$site_country.'/rss/topgrossingapplications/limit=100/xml');
