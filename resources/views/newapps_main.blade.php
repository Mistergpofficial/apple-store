...
<head>
    ...
    <title><?php echo config('siteconfig.newapps_title') ?> - <?php echo config('siteconfig.site_title') ?></title>
    <meta name="description" content="<?php echo config('siteconfig.newapps_title') ?> - <?php echo config('siteconfig.site_title') ?>" />
    ...
    <meta property="og:title" content="<?php echo config('siteconfig.newapps_title') ?> - <?php echo config('siteconfig.site_title') ?>"/>
    <meta property="og:description" content="<?php echo config('siteconfig.newapps_title') ?> - <?php echo config('siteconfig.site_title') ?>"/>
    <!-- CSS and Scripts -->
...
@include('url_slug')
 <div class="container">
     <div class="pagetitle">
         <h1><?php echo config('siteconfig.newapps_title') ?></h1>
     </div>
            ...
            <span class="control last"><i class="material-icons">menu</i></span>
                    <ul class="page-itemlist">
                        @if($latestapps->count() > 0)
                            @foreach($latestapps as $latestapp)
                        <li class="page-item">
                            <div class="pagethumb" data-toggle="tooltip" data-placement="top" title="{{ $latestapp->imname1 }}">
                                <a href="{{ url('/app/' . $latestapp->musicid1[0] . '/' . cano($latestapp->imname1)) }}">
                                    <img data-src="{!! asset($latestapp->bigimage1) !!}" src="{!! asset('public/images/loading.svg') !!}" >
                                </a>
                            </div>
                            <div class="info">
                                <h3>
                                    <a href="{{ url('/app/' . $latestapp->musicid1[0] . '/' . cano($latestapp->imname1)) }}">{{ $latestapp->imname1 }}</a>
                                </h3>
                                <h4>{{ $latestapp->imartist1 }}</h4>
                                <a class="genre" href="{{ url('/category/' . $latestapp->catid1[0] . '/' . cano($latestapp->label)) }}">{{ $latestapp->label }}</a>
