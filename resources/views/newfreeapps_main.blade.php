...
<head>
    ...
    <title><?php echo config('siteconfig.newfreeapps_title') ?> - <?php echo config('siteconfig.site_title') ?></title>
    <meta name="description" content="<?php echo config('siteconfig.newfreeapps_title') ?> - <?php echo config('siteconfig.site_title') ?>" />
    ...
    <meta property="og:title" content="<?php echo config('siteconfig.newfreeapps_title') ?> - <?php echo config('siteconfig.site_title') ?>"/>
    <meta property="og:description" content="<?php echo config('siteconfig.newfreeapps_title') ?> - <?php echo config('siteconfig.site_title') ?>"/>
    <!-- CSS and Scripts -->
        ...
        <div class="pageresults">
            ...
            <span class="control last"><i class="material-icons">menu</i></span>
                        <ul class="page-itemlist">
                            @if($freeapps->count() > 0)
                                @foreach($freeapps as $freeapp)
                            <li class="page-item">
                                <div class="pagethumb" data-toggle="tooltip" data-placement="top" title="{{ $freeapp->imname1 }}">
                                    <a href="{{ url('/app/' . $freeapp->musicid1[0] . '/' . cano($freeapp->imname1)) }}">
                                        <img data-src="{!! asset($freeapp->bigimage1) !!}" src="{!! asset('public/images/loading.svg') !!}" >
                                    </a>
                                </div>
                                <div class="info">
                                    <h3>
                                        <a href="{{ url('/app/' . $freeapp->musicid1[0] . '/' . cano($freeapp->imname1)) }}">{{ $freeapp->imname1 }}</a>
                                    </h3>
                                    <h4>{{ $freeapp->imartist1 }}</h4>
                                    <a class="genre" href="{{ url('/category/' . $freeapp->catid1[0] . '/' . cano($freeapp->label)) }}">{{ $freeapp->label }}</a>
                ...
                @else
                    @php
                        ...
                        $string1 = file_get_contents('https://itunes.apple.com/'.$site_country.'/rss/newfreeapplications/limit=100/xml');
