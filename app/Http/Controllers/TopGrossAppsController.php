...
class TopGrossAppsController extends Controller
{
    public function topAppGross(){
        $topappgross = TopAppsGross::all();
        if(count($topappgross) === 0){
            ...
            foreach ($xmll->entry as $val) {
                ...
                $summary = $val->summary;
                DB::table('top_apps_grosses')->insert([
                    'bigimage1' => $bigimage1,
            ...
            return view('topgrosapps_main',compact('topappgross', 'xmll'));

        }else{
            return view('topgrosapps_main', compact('topappgross'));
        }
    }
